...
class AffichageWishList extends StatefulWidget {
  const AffichageWishList({Key? key}) : super(key: key);

  @override
  State<AffichageWishList> createState() => _AffichageWishListState();
}

class _AffichageWishListState extends State<AffichageWishList> {
  late Future<List> _gameWishlist; // type dynamic
  //late Future<List> _gameWishListIds1;
  // List<int> gameWishListIds = [];
  ...
  Widget build(BuildContext context) {
    ...
    setState(() {
      ...
      _gameWishlist = GameDAO.fetchGetAllGamesDetails(wishlist);
      _gameWishlist.whenComplete(() {
        print('OK');
        _gameWishlist = _gameWishlist.then<List>((values) {
          return values;
        });
      });
    });
